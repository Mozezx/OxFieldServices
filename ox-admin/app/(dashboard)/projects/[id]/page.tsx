...
export default function ProjectDetailPage() {
  ...
  // Atribuição é permitida em 'matched' (1ª vez) e em 'contract_signed' enquanto
  // o contrato ainda não foi assinado pelo worker e não tem escrow.
  ...
  return (
    <>
      ...
      <div className="p-6 space-y-6 max-w-4xl">
        ...
        {/* Info + Status + Ações (tudo no mesmo card) */}
        <Card>
          ...
          {contract && (
            <div className="pt-4 mt-4 border-t border-divider">
              <p className="text-xs text-text-secondary mb-1">Worker atribuído</p>
              <p className="text-white font-medium">
                {contract.worker?.user?.name ?? '—'}
              </p>
              <p className="text-xs text-text-disabled mt-1">
                {contract.signedAt
                  ? 'Contrato assinado pelo worker'
                  : contract.escrow
                    ? 'Escrow ativo'
                    : 'Aguardando assinatura — ainda é possível trocar'}
              </p>
            </div>
          )}

          {/* Ações: transições de status + matching, lado a lado */}
          <div className="pt-4 mt-4 border-t border-divider">
                ...
                {showMatchingAction && (
                  <Link href={`/matching/${project.id}`}>
                    <Button variant="primary" size="sm">
                      ...
                      {canSwap
                        ? 'Trocar worker'
                        : 'Ver candidatos e atribuir worker'}
                    </Button>
                  </Link>
                )}
              </div>
        ...
        {project.phases && project.phases.length > 0 && (
          <Card>
            <h3 className="text-white font-semibold mb-4">Fases do Projeto</h3>
