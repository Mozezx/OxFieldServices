...
class _ProfilePhotoGateState extends ConsumerState<ProfilePhotoGate> {
  ...
  Future<void> _maybeShowPrompt(WorkerProfileModel? worker) async {
    if (worker == null || (worker.avatarUrl ?? '').isNotEmpty) return;
  ...
  Widget build(BuildContext context) {
    ref.listen<AsyncValue<WorkerProfileModel?>>(workerProfileProvider, (previous, next) {
      if (next.hasValue && next.value == null) {
        _dialogScheduled = false;
      }
      next.whenData((w) => unawaited(_maybeShowPrompt(w)));
    });
